...
public class DeepZoomImageVisualizer : Visualizer
{
	...
	public DeepZoomImageVisualizer(string name, IDeepZoomTileSource tileSource)
		: base("DeepZoom " + name)
	{
	...
	private Microsoft.Maui.Graphics.IImage? GetImage(DeepZoomImageTile tile)
	{
	...
	private async void LoadStream(DeepZoomImageTile tile)
	{
		// load in the background
		await Task.Run(async () =>
		{
			var stream = await fileLoader.LoadAsync(tile.Uri);
	...
	private async void DecodeImage(DeepZoomImageTile tile, Stream? stream)
	{
