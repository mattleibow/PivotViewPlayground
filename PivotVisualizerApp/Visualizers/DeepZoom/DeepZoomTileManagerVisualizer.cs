...
public class DeepZoomTileManagerVisualizer : Visualizer
{
	...
	public DeepZoomTileManagerVisualizer(string name, ITileSource tileSource)
	...
	class MauiGraphicsCanvas : ITileDrawingCanvas
	{
	...
	class MauiGraphicsImageLoader : ITileImageFileFetcher
	{
		...
		public async Task<object?> FetchAsync(string uri, CancellationToken cancellationToken = default)
		{
		...
		{
			var stream = await fileFetcher.FetchAsync(uri);
