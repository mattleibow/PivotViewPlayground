...
public class CxmlPivotDataSource : PivotDataSource
{
	...
	private readonly ICxmlFileFetcher fileFetcher;

	private string sourceUri = null!;

	public CxmlPivotDataSource(string sourceUri, ICxmlFileFetcher? fileFetcher = default)
	{
		this.fileFetcher = fileFetcher ?? new DefaultCxmlFileFetcher();
	...
	public string SourceUri
	{
		...
		{
			sourceUri = value;
			BaseUri = UriExtensions.GetBaseUri(value);
		}
	}
	...
	{
		using var stream = await fileFetcher.FetchAsync(SourceUri, cancellationToken);
	...
	private async Task ParseSupplementAsync(CancellationToken cancellationToken)
	{
		...
		using var stream = await fileFetcher.FetchAsync(SupplementUri, cancellationToken);
